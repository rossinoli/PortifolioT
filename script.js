...
// HOTBAR MOBILE — Scrollspy com IntersectionObserver
document.addEventListener('DOMContentLoaded', () => {
  const sections = document.querySelectorAll('section[id], header[id]');
  const hotbarItems = document.querySelectorAll('.hotbar-item');

  if (!sections.length || !hotbarItems.length) return;

  const observerOptions = {
    root: null,
    rootMargin: '-20% 0px -70% 0px', // Aciona quando a seção ocupa a parte superior
    threshold: 0
  };
